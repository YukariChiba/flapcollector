<template>
  <BCard
    class="h-100 mb-0 border-primary"
    style="background: linear-gradient(145deg, #1e2227 0%, #16191d 100%)"
    body-class="p-3 d-flex align-items-center justify-content-between"
  >
    <div>
...
const props = defineProps<{ count: number; minVotes: number; loading: boolean }>()
...
const statusClass = computed(() => {
  if (props.loading) return 'text-muted'
...
const statusTooltip = computed(() =>
  props.minVotes < 3 ? 'Results may include false positives.' : undefined,
)
